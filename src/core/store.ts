...
export class CardStore {
  ...
  constructor(private readonly file = process.env.CARD_STORE_PATH ?? "/data/cards.json") {}
  ...
  private persist(): void {
    ...
    void (async () => { try { await mkdir(dirname(this.file), { recursive: true }); await writeFile(tmp, JSON.stringify(this.all()), "utf8"); await rename(tmp, this.file); } catch { /* in-memory fallback */ } })();
  }
}
