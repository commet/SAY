...
import type { NoticeType, PrivacySummary, SourceAssessment } from "./types.js";
...
class InspectionStore {
  ...
  create(input: Omit<InspectionRecord, "token" | "createdAt" | "expiresAt">, now = new Date()): InspectionRecord {
    ...
    const token = `INSP-${Array.from(randomBytes(10), (byte) => alphabet[byte % alphabet.length]).join("")}`;
