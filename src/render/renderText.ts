...
const classificationLabel = { high: "높음", medium: "보통", low: "사용자 확인 필요" } as const;
...
export function renderCard(card: NoticeCard, now = new Date()): string {
  const classification = card.classification.confirmedByUser ? "사용자 확인" : classificationLabel[card.classification.confidence];
  ...
  const unresolvedMissing = card.missingFields.filter((field) => !resolvedFieldKeys.has(field.fieldKey));
  const openItems = card.actionItems.filter((item) => !["done", "not_applicable"].includes(item.status));
  const openFieldKeys = new Set(openItems.map((item) => item.fieldKey).filter((fieldKey): fieldKey is string => Boolean(fieldKey)));
  const activeReminders = card.reminderSuggestions.filter((reminder) => !reminder.fieldKey || openFieldKeys.has(reminder.fieldKey));
  const nextActiveCheck = [...openItems.map((item) => item.dueAt), ...activeReminders.map((reminder) => /^\d{4}-/.test(reminder.atLabel) ? reminder.atLabel : undefined)].filter((value): value is string => Boolean(value)).sort()[0];
  let out = `[케이스 ${card.code}] ${card.title}\n- 상태: ${caseStatusLabel[card.status]}\n- 안내 종류 판단: ${classification}\n- 출처: ${sourceTrustLabel[card.sourceAssessment.trust]}\n- 개인정보 보호: ${card.privacySummary.total}개 항목 마스킹\n- 자동 삭제: ${formatWhen(card.expiresAt, now)}\n- 업데이트용 버전: ${card.version}`;
  if (nextActiveCheck && Date.parse(nextActiveCheck) < now.getTime()) out = `지난 확인 예정(${formatWhen(nextActiveCheck, now)})이 지났어요. 아직 열린 항목을 확인해 주세요.\n\n${out}`;
  ...
  out += section("아직 비어 있는 내용", unresolvedMissing.map((x) => `- ${x.label} — ${x.whyItMatters}. 이렇게 물어보세요: "${x.suggestedQuestion}"`));
  out += section("⚠ 위험 신호", card.riskSignals.map((x) => `- [${severityLabel[x.severity]}] ${x.label} — ${x.saferNextStep}`));
  out += section("할 일과 현재 상태", card.actionItems.flatMap((x) => [
    `- ${x.label} — ${statusLabel[x.status]}${x.actorName ? ` (${x.actorName})` : ""}`,
    ...(x.resultNote ? [`  ↳ 확인 결과: ${x.resultNote}`] : []),
  ]));
  if (activeReminders.length) out += `\n\n알림으로 걸어두면 좋은 것\n${activeReminders.map((x) => `- ${formatWhen(x.atLabel, now)} — "${x.text}"`).join("\n")}\n(휴대폰 알람이나 캘린더에 직접 등록해 주세요. 사이가 임의로 알림을 보내지는 않아요.)`;
  out += `\n\n가족과 나누기\n- 이 케이스를 가족과 같이 보려면 코드를 공유하세요: ${card.code}\n- 가족은 "사이, ${card.code} 보여줘"라고 하면 돼요.`;
  out += `\n- 이 코드는 비밀번호와 같은 접근 열쇠예요. 신뢰하는 가족에게만 공유해 주세요.`;
  out += `\n\n선택 — 각 항목에 대해 이렇게 말해 주세요\n[내가 확인할게요] [가족에게 물어보기] [확인 결과 남기기] [완료로 표시]`;
  return `${out}\n\n${HOST_HINT}`;
}
