...
export function buildServer(): McpServer {
  ...
  server.registerTool("analyze_notice", {
    description: "Turns a family notice (hospital/checkup, government/welfare, insurance/card/payment, delivery SMS) into an action card with SAY(사이): confirmed facts, missing checks, action items, risk signals, and a family-shareable card code. If the user sent an image, transcribe ALL text verbatim into raw_text and put exact source phrases in quote. Never add facts absent from the original. Mask sensitive identifiers first.",
  ...
  server.registerTool("get_card", {
    description: "Retrieves a SAY(사이) action card by its card code (e.g. SAY-3F7KQ2), so family members can see current facts, remaining items, and who is handling what.",
  ...
  server.registerTool("update_item_status", {
    ...
    inputSchema: { card_code: z.string(), item_label: z.string().max(100).optional(), item_id: z.string().max(10).optional(), new_status: z.enum(["unchecked", "i_will_check", "asked_family", "in_progress", "done", "on_hold", "not_applicable"]), actor_name: z.string().max(20).optional() }, annotations: annotations("Update item status", false),
