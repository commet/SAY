...
export const checklists: Record<NoticeType, ChecklistField[]> = {
  hospital: [
    f("appointment_date", "예약일", true, /검진일|예약일|진료일|(?:건강\s*)?검진.{0,24}예약|예약.{0,24}(?:건강\s*)?검진/, "날짜를 놓치면 일정을 다시 잡아야 할 수 있어요", "검진 예약일이 언제인지 확인해 주세요.", "병원 가기", "병원 갈 준비"),
  ...
  government: [
    ...
    f("required_docs", "필요 서류", true, /필요\s*서류|구비\s*서류|준비\s*서류/, "서류가 빠지면 다시 방문할 수 있어요", "제출할 서류 목록을 확인해 주세요."),
  ...
  delivery_or_smishing: [
    ...
    f("delivery_status", "배송 상태", false, /택배|배송|보관\s*중|반송|주소지|배송지/, "실제 배송 상태와 문자가 일치하는지 확인해야 해요", "공식 배송 조회에서 상태를 확인해 주세요."),
    f("has_link", "링크 포함 여부", true, /https?:\/\/|www\.|(?:bit\.ly|t\.ly|url\.kr|han\.gl|vo\.la|me2\.do)(?:\/|\b)|(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+(?:com|net|org|io|kr|co|xyz|top|shop|club|click|live)(?:\/|\b)/i, "문자 링크는 누르기 전에 주소를 확인해야 해요", "문자 링크 대신 공식 앱에서 직접 확인해 주세요."),
    f("asks_info_or_pay", "개인정보·결제 요구", true, /개인정보|주소.*입력|결제|수수료|관세/, "정보나 결제 요구는 공식 채널인지 먼저 확인해야 해요", "공식 앱이나 고객센터에 같은 요청이 있는지 확인해 주세요."),
  ],
  apartment: [
    f("notice_subject", "안내 주제", true, /안내|공지|작업|점검|공사/, "무엇에 관한 안내인지 알아야 필요한 행동을 정할 수 있어요", "이번 안내의 정확한 작업이나 공지 주제를 확인해 주세요."),
    f("schedule", "일정", true, /일시|기간|예정|부터|까지|\d{1,2}월\s*\d{1,2}일/, "생활에 영향을 주는 시간을 미리 알아야 해요", "시작과 종료 시간을 관리사무소에 확인해 주세요.", "세대 일정 확인하기", "아파트 안내 일정 전"),
    f("affected_area", "영향 범위", true, /대상|동|세대|단지|주차장|승강기|엘리베이터|단수|정전/, "우리 세대가 영향 대상인지 확인해야 해요", "우리 동과 세대가 영향 대상인지 확인해 주세요."),
    f("required_action", "입주민 할 일", false, /협조|이동|비워|잠금|신청|제출|차량/, "사전에 해야 할 일이 있으면 놓치지 않아야 해요", "입주민이 미리 해야 할 일을 확인해 주세요.", "입주민 조치 완료하기", "입주민 조치 마감 전"),
    f("amount", "납부 금액", false, /관리비|분담금|금액|[\d,]+원/, "비용 안내라면 금액과 공식 납부 경로를 확인해야 해요", "정확한 금액과 공식 납부 방법을 확인해 주세요."),
    f("contact", "관리사무소 문의처", false, /관리사무소|문의|연락처|\d{2,3}-\d{3,4}-\d{4}/, "모호한 내용을 공식 관리사무소에 확인할 수 있어요", "관리사무소 공식 번호로 문의해 주세요."),
  ],
  other: [],
};
