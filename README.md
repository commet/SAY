...
사이는 가족이 받은 병원·관공서·보험·택배 안내를 **함께 끝내는 행동 카드**로 바꿉니다. 원문에 있는 사실, 추가 확인이 필요한 빈칸, 남은 할 일과 위험 신호를 분리하고 무작위 `SAY-XXXX-XXXX-XXXX` 카드 코드로 가족이 진행 상태를 이어 볼 수 있습니다.
...
- 서버 측 민감정보 강제 마스킹과 인용문 미저장
- 기본 인메모리 카드 저장 및 최대 24시간 자동 만료
...
- 환경변수: `PORT`(기본 `8080`), `HOST`(기본 `0.0.0.0`), `CARD_TTL_HOURS`(1~24시간, 기본 24), `CARD_STORE_PATH`(명시한 경우에만 파일 저장)
...
- 사용자가 보낸 원문과 인용문은 저장하지 않습니다.
- 주민등록번호, 전화번호, 이메일, 금융번호, 주소, URL 쿼리와 이름은 서버에서도 강제로 마스킹합니다.
- 카드 상태는 기본적으로 메모리에만 보관하며 마지막 접근부터 최대 24시간 뒤 자동 만료됩니다. 서버 재시작 시에도 삭제됩니다.
- 카드 코드는 원문에서 계산하지 않고 약 60비트 무작위 값으로 생성합니다. 코드는 접근 열쇠이므로 신뢰하는 가족에게만 공유해야 합니다.
- 담당자 입력은 이름 대신 엄마·아빠·보호자 같은 일반 역할만 보관합니다.
- 파일 저장은 `CARD_STORE_PATH`를 명시한 경우에만 켜지며, 공모전 배포에서는 사용하지 않습니다.
