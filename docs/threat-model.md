...
| Redacted action case and short redacted result notes | Maximum 24 hours | Memory by default; validated `/data` file in the container |
...
The MCP host is outside the server trust boundary. Host-provided type hints, claimed user confirmations, result notes, tool order and feedback are untrusted inputs. The public network, KC ingress, persisted files and operational logs are separate boundaries.

## Threats, controls and residual risk

| Threat | Primary controls | Residual risk / decision |
|---|---|---|
| Raw PII retained or echoed | Server-side redaction before inspection and result-note storage; no quote fields; URL component removal; risk evidence re-redaction; regression corpus | Novel identifier formats can evade deterministic patterns. Users must still review the redacted preview and confirmation result. |
| Bearer-code guessing | About 80 bits of entropy, fixed 24-hour maximum expiry, IP rate limit, no enumeration endpoint, indistinguishable missing/deleted response | There is no user-bound authorization in 2.1. Anyone who obtains a live code can use it. Share only with trusted family. |
...
SAY 2.1 does not implement OAuth or Key/Token authentication. Selecting either option in PlayMCP would falsely claim a protocol the endpoint cannot complete. The compensating boundary is data minimization, explicit consent, random short-lived bearer codes, no enumeration and immediate deletion.
